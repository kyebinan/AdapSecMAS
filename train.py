...
CURRICULUM: list[CurriculumPhase] = [
    ...
    CurriculumPhase("jam_flood", episode_start=250,  jam_active=True,  flood_active=True,  spoof_active=False),
    CurriculumPhase("all",       episode_start=500, jam_active=True,  flood_active=True,  spoof_active=True),
]
...
def log(message: str, level: LogLevel = LogLevel.INFO) -> None:
    print(f"{level.value}{message}{_RESET}")


def start_message(args: argparse.Namespace, trainer: "MAPPOTrainer") -> str:
    return (
        f"\nAdapSecMAS training\n"
...
def episode_message(episode: int, n_episodes: int, phase: "CurriculumPhase", ep_reward : float, mean_dr: float, proto_sr: float, duration: float,) -> str:
    return (
...
def end_message(best_reward: float) -> str:
    return (
        f"\nTraining complete. Best reward: {best_reward:.2f}\n"
        f"Weights: checkpoints/best.pt\n"
        f"Logs   : logs/train_episodes.csv"
    )

def update_message(episode: int, step: int, metrics: dict) -> str:
    return (
...
def _create_step_row(episode: int, ep_steps: int, info: dict, reward: float, phase: CurriculumPhase,) -> dict:
    step_row = {
        "episode"             : episode,
        "step"                : ep_steps,
...
def _create_episode_row(step_metrics, episode, ep_reward, duration, phase):
    ...
    total_level_steps = sum(m["n_agents_normal"] + m["n_agents_elevated"] + m["n_agents_high"] + m["n_agents_critical"] for m in step_metrics)
...
def parse_args() -> argparse.Namespace:
    ...
    parser.add_argument("--steps-per-episode", type=int, default=512) # TODO : delete this after the refactoring!
...
def train(args: argparse.Namespace) -> None:
    ...
    log(start_message(args, trainer), LogLevel.INFO)
    ...
    for episode in range(1, args.episodes + 1):
        ...
        while not done:
            ...
            # Log step
            step_row = _create_step_row(episode, ep_steps, info, reward, phase)
            ...
            if trainer.should_update():
                ...
                log(update_message(episode, ep_steps, update_metrics), LogLevel.INFO)
        ...
        ep_row = _create_episode_row(step_metrics, episode, ep_reward, duration, phase)
        ...
        log_message = episode_message(episode, args.episodes, phase, ep_reward, mean_dr, proto_sr, duration)
        log(log_message, LogLevel.INFO)
    ...
    log(end_message(best_reward), LogLevel.INFO)
